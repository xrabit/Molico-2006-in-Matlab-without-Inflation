function [q,d,c,Ceq] = Bargaining(mb,ms,m, V, x_0, mupper) ;
...
objective=@(x) -(Utility(x(1))+interp1(m,V,mb-x(2),'spline',"extrap")-interp1(m,V,mb,'spline',"extrap"));
...
%the seller ends up with more than mupper money (outside of the grid)
 ...
 C=[]; %no nonlinear inequalities
  Ceq=-(interp1(m,V,ms+x(2),'spline',"extrap")-interp1(m,V,ms,'spline',"extrap")-cost_function(x(1))); % nonlinear equality: the seller is indifferent between accepting deal and rejecting. 
 end
